...
if [[ -f "$SM_FILE" ]]; then
  ...
  echo "ℹ️  Brak pliku $SM_FILE – pomijam tworzenie ServiceMonitora."
fi
...
# Grafana przez Service (po etykietach)
...
if [[ "${GRAF_SVC_COUNT:-0}" -gt 0 ]]; then
  ...
  # awaryjnie bezpośrednio do poda
...
# Prometheus: najpierw spróbuj przez Service…
...
if [[ "${PROM_SVC_COUNT:-0}" -gt 0 ]]; then
  ...
  # …w przeciwnym razie bezpośrednio do Poda
...
echo "🧪 Prometheus Targets: http://localhost:9090/targets  (szukaj: thesis-app → UP)"
